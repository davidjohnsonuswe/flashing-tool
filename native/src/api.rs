...
pub fn get_battery_status() -> Result<bool>{
    // https://learn.microsoft.com/en-us/windows/win32/api/winbase/ns-winbase-system_power_status
    let mut power_status: SYSTEM_POWER_STATUS = SYSTEM_POWER_STATUS::default();

    unsafe {
        GetSystemPowerStatus(&mut power_status);
        Ok(power_status.BatteryFlag != 128)
    }
}
...
pub fn init() {
    ...
    Battery::AggregateBattery().unwrap().ReportUpdated(&TypedEventHandler::<Battery, IInspectable>::new(|battery, inspectable| {
        ...
        // BATTERY_REPORT_STREAM.read().unwrap().unwrap().add(battery_outcome);
        Ok(())
    })).expect("sdds");
